...
| Document parsing | **Text suite** | txt/md/csv; PDF/Office = option F |
...
Postgres, Redis, RustFS, Qdrant. App needs **`GEMINI_API_KEY`** and a second process: `npm run worker:ingest`.
...
**What:** `document_jobs`, Redis queue, `worker:ingest`, statuses, My documents badges, retry.
...
| P1.3 | Parsers (text first; PDF next) | Unlock content | **Text done; PDF open (F)** |
| P1.4–P1.5 | Chunk + Qdrant + embeddings | RAG store | **Done (B2)** |
| P1.6 | Chat RAG + LLM stream | Product value | **Done (B2+B3)** |
| **P1.7** | **Citations UI** (doc/chunk) | Trust | Open |
| **P1.8** | **PDF/DOCX/XLSX extract** | Real-world files | Open (**F**) |
...
### Option F — **PDF-capable ingest** (recommended product next)

**Ship:** PDF (and ideally DOCX/XLSX) text extraction in the worker → same chunk/embed path.

**Depends on:** A (done). Reuses B2 vector path.

**Exit criteria:** PDF upload → indexed with chunks → answers grounded in that PDF.
