...
| Chat messages | **Done (B2/B3 + sources)** | Stream RAG; persist answer + citation JSONB |
...
| Document parsing | **Text + PDF + OCR + DOCX** | txt/md/csv + PyMuPDF + Tesseract + Mammoth; XLSX open |
...
- Auth tables, `documents`, `chats`, `chat_messages` (incl. `sources` JSONB), `chat_documents`, `chat_message_documents`, `document_jobs`
...
| P1.3 | Parsers (text + PDF + OCR) | Unlock content | **F1 + F1b done** |
| P1.4–P1.5 | Chunk + Qdrant + embeddings | RAG store | **Done (B2)** |
| P1.6 | Chat RAG + LLM stream | Product value | **Done (B2+B3)** |
| **P1.7** | **Sources footer** (doc/chunk) | Trust | **Done** |
| **P1.8** | **DOCX** (Mammoth) | Word files | **Done** |
| **P1.9** | **XLSX** | Spreadsheets | Later |
...
- **F1 (done):** text PDF via **pdf-extract** (PyMuPDF)  
- **F1b (done):** OCR via **PyMuPDF render + Tesseract**, `PDF_OCR_MODE=auto`, 30-page OCR cap, free/local  
- **DOCX (done):** `.docx` via **Mammoth** in the Node worker  
- **Later:** XLSX / legacy `.doc`  

**Exit criteria F1b:** scanned PDF → OCR → `indexed` (or honest `failed`) → chat when text found — **met**.  
**Exit criteria DOCX:** `.docx` → Mammoth → `indexed` (or honest `failed`) → chat — **met**.
...
| If you want… | Choose |
|--------------|--------|
| Better scanned PDFs | **F1b (OCR)** — done |
| Trust / “which source?” | **Sources footer** — done |
| Word (`.docx`) | **DOCX** — done |
| Ship current product safely | **D** |
| Spreadsheets | **XLSX** (later) |
| Trust / polish (layout, stop/regen) | **E** |
| Public documentation hosting | **G** |

**Architect recommendation now:**  
**D** (production hardening) if you want real users; otherwise **E** (UX polish) or **XLSX**.

Historical path: **A → B2 → B3 → F1/F1b → sources → DOCX**.
...
- [x] OCR for scanned PDFs (F1b)  
- [x] Citation UI (sources footer)  
- [x] DOCX extract (Mammoth)  
...
- ~~`Do A`~~ / ~~`Do B2`~~ / ~~`Do B3`~~ / ~~`Do sources`~~ / ~~`Do DOCX`~~ — **done**  
- `Do XLSX` — spreadsheet extract (later)  
